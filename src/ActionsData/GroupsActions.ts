...
export function returnGroupsMsgErrors(res: Response, index: number, errors: any = undefined) {
  const msgs = [
  ...
  if (index !== 0) status = ([1, 2, 4, 5, 7, 8, 10, 11].includes(index)) ? 422 : 404;
...
export default { returnGroupsMsgErrors };
