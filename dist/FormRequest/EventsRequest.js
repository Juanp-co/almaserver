...
function validateRegister(data) {
    ...
    if (!data.initHour || !Validations_1.checkHour(data.initHour)) {
        errors.push(GlobalFunctions_1.setError('Disculpe, pero indicar la hora (correcta) de inicio para el evento.', 'initHour'));
    }
    else {
    ...
    if (!data.endHour || !Validations_1.checkHour(data.endHour)) {
        errors.push(GlobalFunctions_1.setError('Disculpe, pero indicar una hora (correcta) de finalización del evento.', 'endHour'));
    }
    else {
    ...
    if (!data.toRoles || typeof data.toRoles !== 'object' || data.toRoles.length === 0) {
        errors.push(GlobalFunctions_1.setError('Disculpe, pero debe seleccionar los roles para este evento.', 'toRoles'));
    }
    else {
