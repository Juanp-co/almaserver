...
async function getReferrals(req, res) {
    try {
        ...
        const user = await (0, UsersActions_1.getData)(_id, { _id: 1, names: 1, lastNames: 1, document: 1, referred: 1, gender: 1 });
        if (!user) {
        ...
        if (ret.user.referred && (0, Validations_1.checkObjectId)(ret.user.referred)) {
            ret.user.referred = await (0, UsersActions_1.getData)(ret.user.referred, { _id: 1, names: 1, lastNames: 1, document: 1, phone: 1, gender: 1 });
        }
        else
            ret.user.referred = null;
        const data = await Referrals_1.default.findOne({ _id }, { members: 1 }).exec();
        if (data) {
            ret.members = await (0, ReferralsActions_1.default)(data.members);
